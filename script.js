...
function createItemEl(columnEl, column, item, index) {
  let newItem = item.replace(/\r?\n/g, "").trim();
  // List Item
  const listEl = document.createElement("li");
  listEl.classList.add("drag-item");
  listEl.innerHTML = `<span class='text-item'>${newItem}</span> <div class='icons'><svg xmlns="http://www.w3.org/2000/svg" class="icon-edit icon-edit-${column}" id='${index}'  onclick='editItemHamndler(event)'fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
